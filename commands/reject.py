...
class Reject(Command):
    def respond(self):
        ...
        if request_index not in requests:
            return "You have no incoming challenge request with ID {}, {}.".format(request_index, self.name)

        # Reject the request and delete its data
        rejected = str(Challenge(*(database.query("""
            SELECT *
            FROM challenges
            WHERE index = {};
        ...
        return "{} has rejected the following challenge:\n\n{}".format(self.name, rejected)
